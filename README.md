...
| カテゴリ | 技術 | 選定理由 |
|---|---|---|
| フロントエンド | Next.js 16 (App Router) / TypeScript | フルスタック対応・型安全・最新のサーバーコンポーネント活用 |
...
| 制作協力 | Claude AI | 作業効率化、コーディングの正確さ |
...
- スケルトンUIによるローディング表示（レイアウトシフト防止）
- 空状態のイラスト＋CTAによるオンボーディング
- ホバー・タップ時のマイクロインタラクション
- ダークモード対応（システム設定に自動連動）
- ドラッグ&ドロップによるトラック並び替え（スマホ長押し対応）
- セットリスト詳細画面に平均BPM表示
...
### ダークモード対応（Tailwind CSS v4 × システム設定連動）

DJが暗い会場で使うことを想定し、`prefers-color-scheme: dark` によるシステム設定自動連動のダークモードを実装。Tailwind v4 の `dark:` プレフィックスで全コンポーネントに対応し、CSSカスタムプロパティ（`--color-surface` 等）のメディアクエリ上書きと組み合わせることで、デザイントークン単位でのテーマ切り替えを実現しました。

### ドラッグ&ドロップ並び替えとモバイル対応

`@dnd-kit/sortable` を採用し、トラック一覧のドラッグ&ドロップ並び替えを実装。並び替え後はフロントで楽観的更新（即座にUIを反映）し、バックグラウンドで一括PATCHリクエストを送る設計で操作感を最大化しました。

スマートフォンではスクロールとドラッグが競合する問題に対し、`TouchSensor` に250msの起動ディレイ（長押し）を設定しつつ、ドラッグハンドルに `touch-action: none` を適用することで、直感的かつ誤操作しにくい操作性を実現しています。
...
| スマホUI最適化 | スワイプ削除・ボトムシート編集・FABボタン |
